...
+function ($) {
    ...
    $.fn.extend({
        /**
         * @param {Object} op: {type:GET/GET, url:ajax请求地址, data:ajax请求参数列表, callback:回调函数 }
         */
        ajaxUrl: function(op) {
    ...
    Navtab.prototype.reloadForm = function(clearQuery, option) {
        ...
        if ($tab && $panel) {
            ...
            if ($pagerForm && $pagerForm.length) {
                options.type = options.type || $pagerForm.attr('method') || 'GET'
...
+function ($) {
    ...
    Bjuiajax.prototype._ajaxform = function($form, options) {
        ...
        if (BJUI.isIE(8) && !options.type) {
            if (!$form[0].getAttributeNode('method').specified) options.type = 'GET'
        }
        options.type = options.type || $form.attr('method') || 'GET'
...
+function ($) {
    ...
    Datagrid.DEFAULTS = {
        ...
        GETData        : null,     // Send other data to server
        ...
        dropOptions     : {
            ...
            afterDrop   : 'GET'    // GET(Ajax) the current page data to server, request type = editType. Optional ('GET' | function), If set a function, doesn't GET
        },
        tdTemplate      : '',
        ...
        editType        : 'GET',   // Ajax request method of edit, Optional 'GET' | 'GET' | 'raw', (Optional 'raw' is GET raw data)
        ...
        delType         : 'GET',   // Delete URL of ajax request method
        ...
        jsonPrefix      : '',       // JSON object key prefix, for GET data
    ...
    Datagrid.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            loadData: function(data, refreshFlag) {
                ...
                if (options.GETData) {
                    if (typeof options.GETData === 'string') {
                        if (options.GETData.trim().startsWith('{')) options.GETData = options.GETData.toObj()
                        else options.GETData = options.GETData.toFunc()
                    }
                    if (typeof options.GETData === 'function') {
                        options.GETData = options.GETData.apply()
                    }
                    if (typeof options.GETData === 'object') {
                        if (!data) data = options.GETData
                        else data = $.extend({}, options.GETData, data)
                    }
                }
            ...
            quickSort: function(model) {
                ...
                var $th = model.th, data = that.data, allData = that.allData, GETData, direction, name = model.name, type = model.type, $ths = that.$thead.find('> tr > th.datagrid-quicksort-th')
                ...
                if (options.sortAll) {
                    if (options.local === 'remote') {
                        GETData = that.$element.data('filterDatas') || {}
                        GETData[BJUI.pageInfo.pageSize]    = that.paging.pageSize
                        GETData[BJUI.pageInfo.pageCurrent] = that.paging.pageCurrent
                        
                        if (direction) {
                            GETData[BJUI.pageInfo.orderField]     = name
                            GETData[BJUI.pageInfo.orderDirection] = direction
                        } else {
                            GETData[BJUI.pageInfo.orderField]     = ''
                            GETData[BJUI.pageInfo.orderDirection] = ''
                        }
                        
                        GETData['orders'] = getOrders().join(',')
                        that.orders = {orders : GETData['orders']}
            ...
            quickFilter: function(model, filterDatas) {
                ...
                var tools = this, $th = model.th, data = that.data, allData = that.allData, name = model.name, GETData, fDatas, msgs = []
    ...
    Datagrid.prototype.refresh = function(filterFlag) {
        var that = this, options = that.options, tools = that.tools, isDom = that.isDom, pageInfo = BJUI.pageInfo, paging = that.paging, GETData = {}
        ...
        if (!options.GETData || !options.GETData[pageInfo.pageSize]) {
            GETData[pageInfo.pageSize]    = paging.pageSize
            GETData[pageInfo.pageCurrent] = paging.pageCurrent
        }
        if (options.initFilter && typeof options.initFilter === 'object') {
            if (typeof options.initFilter === 'object' && options.initFilter && options.jsonPrefix) {
                $.each(options.initFilter, function(k, v) {
                    GETData[options.jsonPrefix +'.'+ k] = v
                })
            }
            that.doInitFilter = true
        }
        if (filterFlag)
            $.extend(GETData, that.$element.data('filterDatas') || {})
        
        tools.loadData(GETData, true)
    ...
    Datagrid.prototype.filter = function(data) {
        ...
        if (data && typeof data === 'object') {
            if (data.clearOldGETData || !options.GETData)
                options.GETData = {}
            
            $.extend(options.GETData, data)
        }
                ...
                .off('drop.bjui.basedrag').on('drop.bjui.basedrag', function(e, target) {
                    ...
                    if (afterDrop && that.afterDrop) {
                        if (typeof afterDrop === 'function') {
                            ...
                            var GETData = {pageSize:that.paging.pageSize, pageCurrent:that.paging.pageCurrent}, type = options.editType, opts = {url:options.dropOptions.dropUrl, type:'GET', reload:false}
                            ...
                            if (GETData) {
                                GETData = JSON.stringify(options.dropOptions.paging && options.paging ? GETData : GETData['list'])
                                
                                if (type && type === 'raw') {
                                    opts.data = GETData
                                    opts.contentType = 'application/json'
                                } else {
                                    opts.data = {json:GETData}
                                    type && (opts.type = type)
                                }
    ...
    Datagrid.prototype.delRows = function(rows) {
        ...
        var doDel = function() {
            ...
            // remote delete
            if (options.delUrl) {
                var GETData = [], callback = options.delCallback
                ...
                $trs.not($addTrs).each(function() {
                    ...
                    if (options.delPK) {
                        GETData.push(data[options.delPK])
                    } else {
                        if (options.jsonPrefix) {
                            delData = {}
                        ...
                        GETData.push(delData)
                    }
                })
                
                if (typeof callback === 'string') callback = callback.toFunc()
                
                var type = options.delType, opts = {url:options.delUrl, data:(options.delPK ? [{ name:options.delPK, value:GETData.join(',') }] : JSON.stringify(GETData)), type:'GET', okCallback:callback || function(json) { delEnd() }}
    ...
    Datagrid.prototype.saveAll = function($trs) {
        var that = this, options = that.options, keys = options.keys, callback = options.editCallback, $tr, data, data_index, datas = [], changeData, tempData, GETData = [], returnData = [], len = $trs && $trs.length
        ...
        if (!len) {
            ...
            if (callback) {
                ...
                callback = function(json) {
                    ...
                    that.tools.afterSave($trs, GETData)
                }
            }
            if (that.options.saveLocal) {
                ...
                if (that.tools.beforeSave($trs, datas)) {
                    var type = options.editType, opts = {url:options.editUrl, data:JSON.stringify(GETData), type:'GET', okCallback:callback}
    ...
    Datagrid.prototype.updateEdit = function($tr, $btn) {
        var that = this, options = that.options, keys = options.keys, callback = options.editCallback, data, datas = [], changeData, tempData, GETData = [], returnData, data_index = $tr.index()
    ...
    Datagrid.prototype.dialogEdit = function($tr, isAdd, data) {
        ...
        var onLoad = function($dialog) {
            ...
            $save.click(function() {
                var changeData, data, datas = [], GETData, returnData, callback = options.editCallback
                ...
                $form.isValid(function(v) {
                    if (v) {
                        ...
                        if (options.jsonPrefix) {
                            ...
                            $.each(data, function(name, value) {
                                if (!that.tools.isGridData(name))
                                    GETData[options.jsonPrefix +'.'+ name] = value
                            })
                        } else {
                            GETData = $.extend({}, data)
                            
                            for (var key in that.options.keys)
                                delete GETData[that.options.keys[key]]
                        }
    ...
    Tags.prototype.TOOLS = function() {
        ...
        var tools = {
            ...
            doQuery: function() {
                ...
                GETData[options.term] = term
                $.extend(GETData, typeof options.data === 'object' && options.data)
